 ...
 * @spdif: SAI spdif iec60958 protocol. set at init
...
static int stm32_sai_pcm_copy_spdif(struct snd_pcm_substream *substream,
				    ...
				    void *buf, unsigned long bytes)
{
	...
	unsigned int i = sai->spdif_frm_cnt;

	if (copy_from_user(ptr, (void __user *)buf, bytes))
		return -EFAULT;

	do {
		*ptr = ((*ptr >> 8) & 0x00ffffff);

		/* status bit settings for consumer mode.
		 * word length: 24 bits
		 * sampling frequency: not indicated
		 * TBD: use AES/IEC958 CS bit defines from asoundef.h
		 */
		if (i == 24 || i == 32 || i == 33 || i == 35)
			*ptr |= 0x04000000;
