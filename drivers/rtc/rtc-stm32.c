/*
 * STMicroelectronics STM32 Real Time Clock (RTC) driver
 *
 ...
 *
 * stm32_rtc driver is free software; you can redistribute it and/or modify
 * it under the terms of the GNU General Public License version 2 as published
 * by the Free Software Foundation.
 *
 * stm32_rtc driver is distributed in the hope that it will be useful, but
 * WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or
 * FITNESS FOR A PARTICULAR PURPOSE. See the GNU General Public License for more
 * details.
 *
 * You should have received a copy of the GNU General Public License along with
 * stm32_rtc driver. If not, see <http://www.gnu.org/licenses/>.
 */
#include <linux/bcd.h>
#include <linux/clk.h>
...
static int stm32_rtc_probe(struct platform_device *pdev)
{
	...
	if (!of_device_is_compatible(pdev->dev.of_node, "st,stm32mp1-rtc")) {
	...
	if (of_device_is_compatible(pdev->dev.of_node, "st,stm32mp1-rtc")) {
		...
		if (rtc->wakeirq_alarm < 0) {
			...
			dev_warn(&pdev->dev, "no alarm wakeup irq: ret=%d\n",
				 ret);
		} else {
			...
			if (ret) {
				dev_warn(&pdev->dev,
					 "alarm can't wake up the system\n");
	...
	if (stm32_rtc_reg_is_defined(regs.verr)) {
