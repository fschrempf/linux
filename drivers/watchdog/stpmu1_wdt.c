				...
				unsigned int timeout)
{
	...
	ret = regmap_write(wdt->pmic->regmap, WATCHDOG_TIMER_REG, timeout);

	if (ret)
		dev_err(wdt->pmic->dev, "Failed to set watchdog timeout (err = %d)\n",
			ret);
	else
