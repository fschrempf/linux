...
static void stm32_mdma_start_transfer(struct stm32_mdma_chan *chan)
{
	...
	/*
	 * In case of M2D cyclic transfer trigerred by STM32 DMA, the MDMA do
	 * not have to clear the transfer complete flag of the 1st period of
	 * data as in this case the 1st request is genetated by
	 * a dummy transfer in order to fill the SRAM before the first real
	 * request from HW to STM32 DMA
	 */
	if (chan_config->m2m_hw && chan->desc->cyclic &&
	    chan->desc->dir == DMA_MEM_TO_DEV) {
		stm32_mdma_write(dmadev, STM32_MDMA_CMAR(id), 0);
		stm32_mdma_write(dmadev, STM32_MDMA_CMDR(id), 0);
	} else {
					    ...
					    struct dma_tx_state *state)
{
	...
	/*
	 * In case of M2D transfer triggered by STM32 DMA, the MDMA has always
	 * one period in advance in cyclic mode. So, we have to add
	 * 1 period of data to return the good residue to the client
	 */
	if (chan_config->m2m_hw && chan->desc->cyclic &&
	    chan->desc->dir == DMA_MEM_TO_DEV)
		residue += chan->desc->node[0].hwdesc->cbndtr;
...
static int stm32_mdma_probe(struct platform_device *pdev)
{
	...
	of_property_read_u32(of_node, "st,sram-size", &dmadev->sram_size);
